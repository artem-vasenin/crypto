...
// LocalOrderBook хранит L2-книгу ордеров в памяти
type LocalOrderBook struct {
	bids map[float64]float64
	asks map[float64]float64
}

// OrderBookCache обеспечивает thread-safe доступ к L2-стаканам
...
// WSClient управляет подключением к Bybit WS
	...
	for {
		...
		default:
			...
			if err := ws.conn.ReadJSON(&msg); err != nil {
				...
				log.Printf("[WS ERROR] Read failure: %v", err)
