...
"execution": {
  ...
  "trailing_pct": 1.0,
  "trailing_min_move_pct": 0.2,
  "trailing_check_interval": "5s",
  "trailing_price_max_age": "5s",
...
При перезапуске бот восстанавливает позиции через REST, определяет позиции своего направления как `Managed=true` и заново подписывает их на public ticker. Это позволяет продолжить управление уже открытой позицией без свежего screening JSON.
...
## Важное уточнение по directional analysis

Структуры `HH+LL` и `LH+HL` считаются **конфликтующими**, а не «50% bullish/bearish». Такой рынок не получает положительный directional score только за счёт одной стороны формации.

Для `long` и `short` конфликт на `1h` или `4h` теперь является hard block. Это сознательно консервативное правило для текущего research-этапа: сначала собираем статистику, затем решаем, нужны ли отдельные правила для breakout/expansion.

В candidate добавлен `context`:

```json
{
  "direction": "bullish|bearish|conflict|neutral",
  "regime": "trend|expansion|uncertain",
  "htf_conflict": true,
  "local_resistance": 0.0,
  "distance_to_local_resistance_pct": 0.0
}
```

`local_resistance` строится из локальных pivot на 5m/15m/30m и используется как дополнительная защита от позднего входа. Для long блокируется комбинация сильного 24h импульса, RSI 15m >= 68 и очень близкого локального сопротивления.

Также `price_vs_ema10_15m_pct` теперь рассчитывается фактически по EMA10 закрытых 15m свечей вместо постоянного нуля.

### Trailing stop

Управление уже открытыми позициями **не зависит от screening JSON**. Отдельный position manager работает каждые `trailing_check_interval` и использует свежую `MarkPrice` из public WebSocket.

Если WebSocket price старше `trailing_price_max_age`, trailing пропускает итерацию. Старый screening price не используется как fallback.

Trailing больше не активируется сразу после fill. Сначала позиция должна пройти **1R** — расстояние от entry до первоначального SL. Только после этого trailing начинает подтягивать защитный стоп.

Чтобы SL не двигался вслед за микрошумом, изменение SL меньше `trailing_min_move_pct` не отправляется на Bybit. По умолчанию это `0.2%`. При этом новый экстремум запоминается, поэтому несколько небольших движений могут накопиться до следующего значимого изменения SL.

SL может двигаться только в безопасную сторону: для long только вверх, для short только вниз. Ухудшение существующего SL запрещено.

Первоначальный SL остаётся структурным: он ставится за соответствующим pivot и минимум на `1.5 * ATR(1h)`, а `ValidateStopLoss` дополнительно требует не менее `0.8%` дистанции и не более `max_stop_loss_pct`. Поэтому обычный микрошум не должен заменять исходный структурный SL сразу после входа.

Логи `[TRAILING]` показывают цену, экстремум, entry, старый SL, кандидатный SL, величину изменения, причину и результат API-операции. `[PRICE WARN]` показывает пропуски trailing из-за отсутствия свежей WebSocket MarkPrice.
